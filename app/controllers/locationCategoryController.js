app.controller('locationCategoryController', function ($scope, sessionService, locationCategoryService, adminBoundaryService, $location) {
    ...
    $scope.district_cat = [];
    $scope.province_cat = [];
    $scope.interest_province = [];
    $scope.interest_district = [];
    $scope.interest_dsd = [];
    $scope.interest_gnd = [];
    $scope.gnd_name = "";
    $scope.dsd_name = "";
                ...
                locationCategoryService.getDsdCategories($scope.dsd_id).then(function (obj) {
                    ...
                    locationCategoryService.getDistrictCategories($scope.district_id).then(function (obj) {
                        ...
                        locationCategoryService.getProvinceCategories($scope.province_id).then(function (obj) {
                            ...
                            locationCategoryService.getInterestProvinceCategories($scope.province_id).then(function (obj) {
                                console.log(obj);
                                $scope.interest_province = obj.data.records;
                                locationCategoryService.getInterestDistrictCategories($scope.district_id).then(function (obj) {
                                    $scope.interest_district = obj.data.records;
                                    locationCategoryService.getInterestDsdCategories($scope.dsd_id).then(function (obj) {
                                        $scope.interest_dsd = obj.data.records;
                                        locationCategoryService.getInterestGndCategories($scope.gnd_id).then(function (obj) {
                                            $scope.interest_gnd = obj.data.records;
                                        });
                                    });
                                });
                            });
